...
def getWordScore(word, n):
    ...
    if len(word) == 0:
        return 0  # returns score 0 is word is an empty string
    else:
        for letter in word:
            score += SCRABBLE_LETTER_VALUES[letter] 
    if n == len(word):
        return score * len(word) + 50  # adding first turn bonus
    else:
...
def updateHand(hand, word):
    ...
    returns: dictionary (string -> int)
    """
    updatedHand = hand.copy()  # creates clone of hand
    for char in word:
        updatedHand[char] -= 1  # decrements frequency of used letters
    return updatedHand  # returns updated hand
